...
async def sendNormalMessage(url, title="This is a Test Message from CTRL - MANAGEMENT", description="Just a Plain ol Test message", color_hex="#FFFFFF"):
...
async def sendFileMessage(url, title="This is a Test Message from CTRL - MANAGEMENT", description="Just a Plain ol Test message" , file_name="", file_path=None, color_hex="#FFFFFF"):
    async with aiohttp.ClientSession() as session:
        webhook = Webhook.from_url(url, session=session)
        embed = discord.Embed(title=title, description=description, timestamp=datetime.now(), color=discord.Color.from_str(color_hex))
